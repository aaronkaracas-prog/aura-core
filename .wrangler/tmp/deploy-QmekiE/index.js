...
// src/index.js
var BUILD = "AURA_CORE__AUTONOMY_LAYERS__EVIDENCE_ALLOWLIST_HOSTCAPS_OPERATOR_INTENT_PAUSE__MEMORY_SUBSTRATE_V1__REGISTRY_AUDIT__13";
var STAMP = (/* @__PURE__ */ new Date()).toISOString();
var autonomyTickKey = `autonomy:tick:v1`;
...
var index_default = {
  async fetch(request, env) {
    try {
      ...
      const operatorHeader = request.headers.get("x-operator-token") || request.headers.get("X-Operator-Token") || "";
...
Tip: In UI, use ALL-CAPS commands. For multi-line ops, use PowerShell batch mode.`;
      ...
      const auditEventKey = /* @__PURE__ */ __name((seq) => `audit:${REGISTRY_VERSION}:event:${seq}`, "auditEventKey");
      ...
      const auditWrite = /* @__PURE__ */ __name(async (env2, event) => {
        const ts = nowIso();
        const seqRaw = await env2.AURA_KV.get(auditSeqKey);
        const seq = Number(seqRaw || 0) + 1;
        await env2.AURA_KV.put(auditSeqKey, String(seq));
        const payload = { seq, ts, ...event };
        await env2.AURA_KV.put(auditEventKey(seq), JSON.stringify(payload));
        return payload;
      }, "auditWrite");
      const auditList = /* @__PURE__ */ __name(async (env2, limit = 50) => {
        const seqRaw = await env2.AURA_KV.get(auditSeqKey);
        const seq = Number(seqRaw || 0);
        const out = [];
        const start = Math.max(1, seq - limit + 1);
        for (let i = start; i <= seq; i++) {
          const s = await env2.AURA_KV.get(auditEventKey(i));
          if (!s) continue;
      ...
      const registryPut = /* @__PURE__ */ __name(async (env2, type, obj) => {
        ...
        await auditWrite(env2, { action: "REGISTRY_PUT", type, id });
      ...
      if (bodyTrim === "SHOW_BUILD") {
        return jsonReply(
          JSON.stringify({ build: BUILD, stamp: (/* @__PURE__ */ new Date()).toISOString() }, null, 2)
        );
      }
      ...
      const isAllowedForHost = /* @__PURE__ */ __name((cmd) => {
