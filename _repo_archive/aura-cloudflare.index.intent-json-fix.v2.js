// AURA CORE Ã¢â‚¬â€ CANONICAL REBUILD (FULL FILE REPLACEMENT)
...
function routeInputBeforeClaimGate(ctx, input) {
  ...
  // Natural language Ã¢â€ â€™ intent-first
  return { intentFirst: true, text: input };
}
// === END UI_ROUTER_ENTRY ===

// Canonical build/stamp (rendered in UI header and returned by SHOW_BUILD)
const BUILD = 'AURA_CORE__AUTONOMY_LAYERS__EVIDENCE_ALLOWLIST_HOSTCAPS_OPERATOR_INTENT_PAUSE__MEMORY_SUBSTRATE_V1__REGISTRY_AUDIT__13__INTENTJSON_FIX_V2';
...
async function naturalLanguageReply(input, env, activeHost) {
  ...
  const identity = (await intentGet("identity")) || "I am Aura Ã¢â‚¬â€ an autonomous, evidence-first control-plane for ARK Systems. Host-scoped. Persistence-backed. Neutral and non-coercive.";
  ...
  <div class="wrap">
    <div class="topbar">
      <div class="brand">
        ...
        <div class="s">UI route: <code class="k">/ui</code> Ã¢â€ â€™ POST <code class="k">/chat</code></div>
      </div>
      <div class="meta">
        <div class="row"><span class="dot"></span><span id="status">Ready</span></div>
        <div class="row"><span>Build:</span><span id="build">Ã¢â‚¬Â¦</span></div>
        <div class="row"><span>Stamp:</span><span id="stamp">Ã¢â‚¬Â¦</span></div>
        <div class="row"><span>Local:</span><span id="localtime">Ã¢â‚¬Â¦</span></div>
      </div>
    </div>
  ...
  (async ()=>{
    try{
      ...
      if(res && res.ok && res.reply && res.reply.build){
        ...
        stampEl.textContent = res.reply.stamp || 'Ã¢â‚¬â€';
      }else{
        buildEl.textContent = 'unknown';
        stampEl.textContent = 'Ã¢â‚¬â€';
      }
    }catch(e){
      buildEl.textContent = 'unknown';
      stampEl.textContent = 'Ã¢â‚¬â€';
...
export default {
  async fetch(request, env) {
    ...
    // ----------------------------
    // Operator auth (explicit only) Ã¢â‚¬â€ computed early (used by NL responses)
    // ----------------------------
    ...
    if (bodyTrim && !isBatchLike && !hasClaimTrigger(bodyTrim)) {
      const msg =
        `Aura (control-plane) Ã¢â‚¬â€ ${BUILD} @ ${STAMP}\n` +
...
function __parseRegistryGetArgs(line, lines, idx, allowedCommands) {
  ...
  // No id on the same line Ã¢â€ â€™ collect JSON below
...
const __withOperatorHeaders = (headers, isOperator, operatorHeader) => {
  const out = { ...(headers || {}) };
...
// Claim-gate (batch mode): registry writes for a domain require VERIFIED_FETCH evidence for that domain's host.
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // ----------------------------
    // Intent (batch mode) Ã¢â‚¬â€ emit cmd entries (no early return)
    // ----------------------------
    if (line.startsWith("INTENT_ADD")) {

// Accept either legacy positional form:
//   INTENT_ADD <host> <tag> <text>
// or JSON envelope form:
//   INTENT_ADD {"host":"auras.guide","tag":"autonomy","text":"...","consent":"EXPLICIT_COMMAND"}
const rest = line.replace(/^INTENT_ADD\s*/,"").trim();
let host = "";
let tag = "";
let txt = "";
let consent = "EXPLICIT_COMMAND";

if (rest.startsWith("{")) {
  const obj = safeJsonParse(rest);
  if (!obj || typeof obj !== "object") { push("INTENT_ADD", "BAD_REQUEST"); continue; }
  host = String((obj.host || activeHost || "")).toLowerCase();
  tag  = String((obj.tag || (obj.intent && obj.intent.tag) || "")).toLowerCase();
  txt  = String((obj.text || (obj.intent && obj.intent.text) || obj.note || (obj.intent && obj.intent.note) || ""));

  if (obj.consent) consent = String(obj.consent);
} else {
  const m = rest.match(/^(\S+)\s+(\S+)\s+(.+)$/);
  if (!m) { push("INTENT_ADD", "BAD_REQUEST"); continue; }
  host = String(m[1]).toLowerCase();
  tag  = String(m[2]).toLowerCase();
  txt  = String(m[3]);
}

if (!host || !tag) { push("INTENT_ADD", "BAD_REQUEST"); continue; }
...
//   INTENT_GET <host> <tag>
// or JSON envelope form:
//   INTENT_GET {"host":"auras.guide","tag":"autonomy"}
const rest = line.replace(/^INTENT_GET\s*/,"").trim();
let host = "";
let tag = "";
...
if (rest.startsWith("{")) {
  const obj = safeJsonParse(rest);
  if (!obj || typeof obj !== "object") { push("INTENT_GET", "BAD_REQUEST"); continue; }
  host = String((obj.host || activeHost || "")).toLowerCase();
  tag  = String((obj.tag || (obj.filter && obj.filter.tag) || "")).toLowerCase();
} else {
  const parts = rest.split(" ").filter(Boolean);
  host = String(parts[0] || "").toLowerCase();
  tag  = String(parts[1] || "").toLowerCase();
}
...
// Accept either legacy positional form:
//   INTENT_CLEAR <host> <tag>
// or JSON envelope form:
//   INTENT_CLEAR {"host":"auras.guide","tag":"autonomy"}
const rest = line.replace(/^INTENT_CLEAR\s*/,"").trim();
let host = "";
let tag = "";
...
if (rest.startsWith("{")) {
  const obj = safeJsonParse(rest);
  if (!obj || typeof obj !== "object") { push("INTENT_CLEAR", "BAD_REQUEST"); continue; }
  host = String((obj.host || activeHost || "")).toLowerCase();
  tag  = String((obj.tag || (obj.filter && obj.filter.tag) || "")).toLowerCase();
} else {
  const parts = rest.split(" ").filter(Boolean);
    ...
    if (line.startsWith("DEPLOYER_CALL ")) {
      ...
      try {
        ...
        const resp = await __serviceFetch(svc, {
          ...
          headers: __withOperatorHeaders(((reqObj.headers && typeof reqObj.headers === "object") ? reqObj.headers : {}), isOperator, operatorHeader),
  ...
  try {
    ...
    for (const line of lines) {
      if (line.startsWith("INTENT_ADD")) {
        ...
        // JSON envelope: INTENT_ADD {"host":"auras.guide","tag":"autonomy","text":"...","consent":"EXPLICIT_COMMAND"}
        if (rest.startsWith("{")) {
          const obj = safeJsonParse(rest);
          if (!obj || typeof obj !== "object") return jsonReply("BAD_REQUEST");

          const host = String((obj.host || (typeof activeHost !== "undefined" ? activeHost : "") || "")).toLowerCase();
          const tag  = String((obj.tag || (obj.intent && obj.intent.tag) || obj.scope || "autonomy")).toLowerCase();
          const text = String((obj.text || (obj.intent && obj.intent.text) || obj.note || (obj.intent && obj.intent.note) || ""));

          if (!host || !tag || !text) return jsonReply("BAD_REQUEST");
          ...
          const payload = {
            ...
            consent: String(obj.consent || (obj.intent && obj.intent.consent) || "EXPLICIT_COMMAND")
          };

          await env.AURA_KV.put(intentKey(host, tag), JSON.stringify(payload));
          return jsonReply(JSON.stringify({ ok: true, status: "INTENT_SAVED", host, tag }));
        }

        // Legacy positional: INTENT_ADD <host> <tag> <text>
      ...
      if (line.startsWith("INTENT_GET")) {
        ...
        // JSON envelope: INTENT_GET {"host":"auras.guide","tag":"autonomy"}
        if (rest.startsWith("{")) {
          const obj = safeJsonParse(rest);
          if (!obj || typeof obj !== "object") return jsonReply("BAD_REQUEST");
          const host = String((obj.host || (typeof activeHost !== "undefined" ? activeHost : "") || "")).toLowerCase();
          const tag  = String((obj.tag || (obj.filter && obj.filter.tag) || "")).toLowerCase();
          if (!host || !tag) return jsonReply("BAD_REQUEST");
          const stored = await env.AURA_KV.get(intentKey(host, tag));
          return jsonReply(stored ? stored : "INTENT_MISSING");
        }

        // Legacy positional: INTENT_GET <host> <tag>
        const parts = line.split(" ").filter(Boolean);
      ...
      if (line.startsWith("INTENT_CLEAR")) {
        ...
        // JSON envelope: INTENT_CLEAR {"host":"auras.guide","tag":"autonomy"}
        if (rest.startsWith("{")) {
          const obj = safeJsonParse(rest);
          if (!obj || typeof obj !== "object") return jsonReply("BAD_REQUEST");
          const host = String((obj.host || (typeof activeHost !== "undefined" ? activeHost : "") || "")).toLowerCase();
        ...
        // Legacy positional: INTENT_CLEAR <host> <tag>
        const parts = line.split(" ").filter(Boolean);
        const host = (parts[1] || "").toLowerCase();
        const tag = (parts[2] || "").toLowerCase();
        if (!host || !tag) return jsonReply("BAD_REQUEST");
