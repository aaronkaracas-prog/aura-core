...
.hero::after {
  ...
  position:absolute;
  inset:0;
  background:linear-gradient(180deg, rgba(0,0,0,.45) 0%, rgba(0,0,0,.35) 40%, rgba(0,0,0,.70) 100%);
}
.hero h1 {
  position:relative;
  margin:24px;
...
<script>
// Locked hero images (fixed photo IDs; deterministic)
const CITIES = {
  vegas: {
    name: "VEGAS",
    hero: "https://images.unsplash.com/photo-1505761671935-60b3a7427bad?auto=format&fit=crop&w=1800&q=80",
    bg: "#1a0826",
    accent: "#ffb703",
  ...
  malibu: {
    name: "MALIBU",
    hero: "https://images.unsplash.com/photo-1500375592092-40eb2168fd21?auto=format&fit=crop&w=1800&q=80",
    bg: "#021a2d",
    accent: "#38bdf8",
  ...
  florence: {
    name: "FLORENCE",
    hero: "https://images.unsplash.com/photo-1526481280691-90606c2f9a7c?auto=format&fit=crop&w=1800&q=80",
    bg: "#2a0f14",
    accent: "#e11d48",
...
function detectCityFromText(t) {
  const v = (t || "").toLowerCase();
  if (v.includes("malibu")) return CITIES.malibu;
  if (v.includes("florence")) return CITIES.florence;
  if (v.includes("vegas") || v.includes("las vegas") || v.trim() === "lv") return CITIES.vegas;
  return null;
}
...
qInput.addEventListener("keydown", e => {
  if (e.key === "Enter") {
    const city = detectCityFromText(e.target.value) || currentCity;
    applyCity(city);
    load(e.target.value || city.defaultQuery);
  }
});
