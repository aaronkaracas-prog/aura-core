...
<div class="app">
  ...
  <main class="main">
    ...
    <div class="composerWrap">
      <div class="composer">
        <button id="attachBtn" class="iconBtn" title="Attach">⤒</button>
        <button id="micBtn" class="iconBtn" title="Mic">🎙</button>
...
if (line === "FAILURE_MEMORY_PUT") {
  ...
  "SELF_PATCH_PUT",
  "SELF_PATCH_GET",
  "SELF_PATCH_APPLY", "NOT_ALLOWED"); continue; }
  ...
  "SELF_PATCH_PUT",
  "SELF_PATCH_GET",
  "SELF_PATCH_APPLY", { ok: false, error: r.error, note: r.note }); continue; }
  ...
  "SELF_PATCH_PUT",
  "SELF_PATCH_GET",
  "SELF_PATCH_APPLY", { ok: false, error: "BAD_REQUEST", note: "Blank item refused" });
    i += r.consumed;
    continue;
  };
  ...
  "SELF_PATCH_PUT",
  "SELF_PATCH_GET",
  "SELF_PATCH_APPLY", { ok: true, host, item, count: bounded.length });
  i += r.consumed;
  continue;
}
...
if (line === "FAILURE_MEMORY_SET") {
  ...
  "SELF_PATCH_PUT",
  "SELF_PATCH_GET",
  "SELF_PATCH_APPLY", { ok: false, error: "BAD_REQUEST", note: "Expected { items: [...] }" }); continue; }
  ...
  "SELF_PATCH_PUT",
  "SELF_PATCH_GET",
  "SELF_PATCH_APPLY", { ok: true, host, count: bounded.length });
  ...
  continue;
    ...
    if (line === "SELF_PATCH_PUT") {
      ...
      if (!ev || !ev.ok) { push("SELF_PATCH_PUT", "NOT WIRED: VERIFIED_FETCH REQUIRED"); continue; }
    ...
    if (line === "SELF_PATCH_GET") {
      ...
      const nextLine = lines[i + 1];
      const ev = await __getHostEvidence(activeHost);
      if (!ev || !ev.ok) { push("SELF_PATCH_GET", "NOT WIRED: VERIFIED_FETCH REQUIRED"); continue; }
    ...
    if (line === "SELF_PATCH_APPLY") {
      ...
      const nextLine = lines[i + 1];
      const ev = await __getHostEvidence(activeHost);
      if (!ev || !ev.ok) { push("SELF_PATCH_APPLY", "NOT WIRED: VERIFIED_FETCH REQUIRED"); continue; }
      if (!isOperator) { push("SELF_PATCH_APPLY", { ok:false, error:"OPERATOR_REQUIRED" }); continue; }

      // TX1: mutation surface not implemented in-worker yet. This is a controlled stub.
      push("SELF_PATCH_APPLY", { ok:false, host: activeHost, error:"MUTATION_SURFACE_NOT_IMPLEMENTED", next:"Use external mutation bridge / deployer once wired." });
      continue;
    }
