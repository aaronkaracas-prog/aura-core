...
# optional single-pending rule: fail if more than one pending exists
$pendingCount = 0
for ($j = 0; $j -lt $plans.Count; $j++) {
  $pid2 = $plans[$j]
  $p2 = (RegistryGetConfig $pid2).payload
...
# 7) execlog append (small summary string)
