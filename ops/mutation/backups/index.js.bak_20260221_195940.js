  ...
  try {
    ...
    if (supported === "MUTATION_KV_PUT_TEST") {
      ...
      if (!env || !env.AURA_KV) {
        return {
          stage: "NOT_WIRED",
          receipt: { ts: new Date().toISOString(), type: "SELF_PATCH_MUTATION_V1", note: "Missing env.AURA_KV" }
        };
      }

      const host = (ctx && ctx.activeHost) ? String(ctx.activeHost).toLowerCase() : "unknown";
      const key = `SELF_PATCH_MUTATION_TEST__${host}`;
      const payload = { ok: true, host, ts: new Date().toISOString(), note: "proof_kv_put_roundtrip" };
      ...
      return {
        ...
        mutation: { type: "KV_PUT_ROUNDTRIP", key, bytes: got ? String(got).length : 0 },
        receipt: { ts: new Date().toISOString(), type: "SELF_PATCH_MUTATION_V1", note: "KV put/get roundtrip succeeded." }
      };
    }
};
    }

    // NEW: Treat supported as a PATCH_INDEX id and prove we can resolve -> object -> b64 -> JSON
    const env = ctx && ctx.env ? ctx.env : null;
    if (!env || !env.AURA_KV) {
      return {
        stage: "NOT_WIRED",
        receipt: { ts: new Date().toISOString(), type: "SELF_PATCH_TRANSFORM_V1", note: "Missing env.AURA_KV" }
      };
    }
    ...
    if (!objId) {
      return {
        ...
        receipt: { ts: new Date().toISOString(), type: "SELF_PATCH_TRANSFORM_V1", note: `No patch index for ${idxId}` }
      };
    }
    ...
    if (!b64) {
      return {
        ...
        receipt: { ts: new Date().toISOString(), type: "SELF_PATCH_TRANSFORM_V1", note: `No patch object for ${objId}` }
      };
    }
...
const jsonText = new TextDecoder("utf-8").decode(bytes);
    ...
    return {
      ...
      patch: { index_id: idxId, object_id: objId, bytes_b64: String(b64).length, json_ok: !!parsed },
      receipt: {
        ts: new Date().toISOString(),
