...
if (line === "PATCH_INDEX_GET") {
  const id = args?.[0];
  if (!id) { push("PATCH_INDEX_GET","PATCH_INDEX_APPLY", { ok:false, error:"MISSING_ID" }); continue; }
  const key = "patch_index:" + id;
  const b64 = await env.AURA_KV.get(key);
  push("PATCH_INDEX_GET","PATCH_INDEX_APPLY", { ok:true, id, key, present: !!b64, bytes_b64: b64 ? b64.length : 0 });
  continue;
}
    if (line === "PATCH_INDEX_PUT") {
  if (!operator) { push("PATCH_INDEX_PUT", { ok:false, error:"NOT_AUTHORIZED" }); continue; }
