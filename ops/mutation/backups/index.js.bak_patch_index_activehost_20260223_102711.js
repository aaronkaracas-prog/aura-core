...
if (line === "PATCH_INDEX_APPLY") {
  ...
  await env.AURA_KV.put("PATCH_INDEX_ACTIVE__" + host, id);
push("PATCH_INDEX_APPLY", { ok:true, id, key:"PATCH_INDEX_ACTIVE__" + host, note:"PATCH_INDEX_APPLY_SET_ACTIVE" });
  continue;
}
if (line === "PATCH_INDEX_GET") {
  let id = args?.[0];
  if (!id) {
  const activeId = await env.AURA_KV.get("PATCH_INDEX_ACTIVE__" + host);
