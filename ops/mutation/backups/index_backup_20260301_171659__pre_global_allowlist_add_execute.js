...
if (line.startsWith("RUNTIME_COMMAND_DEFINE")) {
  ...
  const body = (typeof nextLine === "string") ? nextLine : "";
...
if (line.startsWith("RUNTIME_COMMAND_EXECUTE")) {
  ...
  try {
    const fn = new Function("env","host","push","input", String(body));
    const input = (typeof nextLine === "string") ? nextLine : null;
    const result = await fn(env, hostKey, push, input);
    push("RUNTIME_COMMAND_EXECUTE", { ok:true, host: hostKey, name, result: (result===undefined?null:result) });
  } catch (e) {
