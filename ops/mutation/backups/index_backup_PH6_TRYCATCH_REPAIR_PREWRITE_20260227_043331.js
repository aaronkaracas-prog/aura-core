...
push("AUTONOMY_LAST_TICK", { ok: true, host: __host, last_tick: (chosen || null) });
  ...
  for (const item of __arr) {
    if (item?.code === "NEEDS_PLANNER_ACTION") {
      __ph6_actions.push({ type: "kv_only", op: "KV_PUT", key: "PH6_RECEIPT__" + __host, value: { code: item.code, detail: item.detail || "", emitted_at: nowIso() } });
    }
  }
          ...
          push("AUTONOMY_REPAIR_PLAN", { ok: true, host: __host, stamp: nowIso(), actions: __out_actions, count: __out_actions.length });continue;
      }
