...
push("AUTONOMY_LAST_TICK", { ok: true, host: __host, last_tick: (chosen || null) });
      ...
      if (line === "AUTONOMY_REPAIR_EXECUTE" || fullLine === "AUTONOMY_REPAIR_EXECUTE" || fullLine.startsWith("AUTONOMY_REPAIR_EXECUTE ")) {
          ...
          try {
            ...
            if (status === 200) {
              const payload = {
                ...
                body_length: 0,
                synthetic: false,
                reason: "AUTONOMY_SELF_VERIFY_FETCH_200",
                ts: nowIso()
              };
              await env.AURA_KV.put(evidenceKey(hostLower), JSON.stringify(payload));
              evPresent = true;
            }
          } catch (_) {
        ...
        if (!evPresent) {
          ...
          results.push({ fixed: "EVIDENCE_PRESENT", key: evidenceKey(hostLower) });
        }
