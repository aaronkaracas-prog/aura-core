...
push("AUTONOMY_LAST_TICK", { ok: true, host: __host, last_tick: (chosen || null) });
  ...
  // Read-only planner: proposes deterministic repairs for selected host.
  const b = (await __readJsonKV(__budgetKey)) || __defaultBudget;
  const c = (await __readJsonKV(__charterKey)) || __defaultCharter;
  ...
  // Baseline planner actions (existing behavior)
  const actions = [];
  if (!evidencePresent) {
  ...
  // === PH6: bridge FAILURE_MEMORY -> kv_only action emission (safe, no writes here) ===
  const __ph6_actions = [];
  try {
    ...
    // planner stays read-only; PH6 emission is best-effort
  }

  const out_actions = (__ph6_actions.length > 0) ? __ph6_actions : actions;
  push("AUTONOMY_REPAIR_PLAN", { ok: true, host: __host, stamp: nowIso(), actions: out_actions, count: out_actions.length });
  continue;
}
            }
          }
