...
push("AUTONOMY_LAST_TICK", { ok: true, host: __host, last_tick: (chosen || null) });
  ...
  let __ph6_actions = [];
  try {
    const __fail = await FAILURE_MEMORY_GET({ host });
      ...
      for (const item of __fail.items) {
        if (item?.code === "NEEDS_PLANNER_ACTION") {
          __ph6_actions.push({
            type: "kv_only",
            op: "KV_PUT",
            key: "PH6_RECEIPT__" + host,
            value: { code: item.code, detail: item.detail || "", emitted_at: new Date().toISOString(), build }
          });
        }
      }
    }
...
if (line === "FAILURE_MEMORY_CLEAR") {
  ...
  push("FAILURE_MEMORY_CLEAR", { ok: true, host, cleared: true, count: __ph6_actions.length });
...
if (line === "INTENT_SIMULATE") {
    ...
    if (line === "PORTFOLIO_STATUS") {
      ...
      const status = {
        ...
        registry_version: REGISTRY_VERSION,
        registries: {
          assets: assetsMeta || { type: "assets", count: __ph6_actions.length, updated_at: null, version: REGISTRY_VERSION },
          domains: domainsMeta || { type: "domains", count: __ph6_actions.length, updated_at: null, version: REGISTRY_VERSION }
        }
      };
