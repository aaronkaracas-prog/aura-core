...
push("AUTONOMY_LAST_TICK", { ok: true, host: __host, last_tick: (chosen || null) });
  ...
  const __ph6_actions = [];
  try {
    const __fail = await FAILURE_MEMORY_GET({ host: __host });
    if (__fail?.items?.length) {
      for (const item of __fail.items) {
        if (item?.code === "NEEDS_PLANNER_ACTION") {
          __ph6_actions.push({
            type: "kv_only",
            op: "KV_PUT",
            key: "PH6_RECEIPT__" + __host,
            value: { code: item.code, detail: item.detail || "", emitted_at: nowIso(), build }
          });
        }
      }
    }
...
// === PH6_PLANNER_ACTION_EMIT_END ===
  const __out_actions = (__ph6_actions.length ? __ph6_actions: __out_actions);
push("AUTONOMY_REPAIR_PLAN", { ok: true, host: __host, stamp: nowIso(), actions, count: __out_actions.length });
        continue;
      }
...
if (line === "FAILURE_MEMORY_CLEAR") {
  ...
  push("FAILURE_MEMORY_CLEAR", { ok: true, host, cleared: true, count: __out_actions.length });
...
if (line === "SELF_PATCH_PUT") {
  const nextLine = lines[i + 1];
  const args = (typeof nextLine === "string") ? nextLine.trim() : "";

  // Packet parser splits on whitespace; JSON payloads are fragile here.
  // Canon: accept a single-line JSON as fallback, OR positional args:
  // SELF_PATCH_PUT <stamp> <mode> <index_id> <patch_object_id> <note_b64>
  let body = null;

  // Try JSON fallback if args looks like JSON (starts with "{")
  if (args && args[0] === "{") {
    try { body = JSON.parse(args); } catch (_) { body = null; }
  }
...
if (line === "INTENT_SIMULATE") {
    ...
    if (line === "PORTFOLIO_STATUS") {
      ...
      const status = {
        ...
        registry_version: REGISTRY_VERSION,
        registries: {
          assets: assetsMeta || { type: "assets", count: __out_actions.length, updated_at: null, version: REGISTRY_VERSION },
          domains: domainsMeta || { type: "domains", count: __out_actions.length, updated_at: null, version: REGISTRY_VERSION }
        }
      };
