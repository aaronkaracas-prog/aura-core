...
// === PH6_PLANNER_ACTION_EMIT_END ===
      ...
      if (line === "AUTONOMY_REPAIR_EXECUTE" || fullLine === "AUTONOMY_REPAIR_EXECUTE" || fullLine.startsWith("AUTONOMY_REPAIR_EXECUTE ")) {
        ...
        if (obj && obj.recordFailure) {
          try {
            const __fmExecKey = "FAILURE_MEMORY__" + String(__host || String(activeHost || "frontdesk.network").toLowerCase());
            await env.AURA_KV.put(__fmExecKey, JSON.stringify([]));
            results.push({ fixed: "FAILURE_MEMORY_CLEARED", key: __fmExecKey });
          } catch (e) {
