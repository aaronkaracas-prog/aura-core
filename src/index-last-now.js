...
async function naturalLanguageReply(input, env, activeHost) {
  ...
  const humanMode = lower.startsWith("human:");
  if (humanMode) {
    lower = lower.slice("human:".length).trim();
  }
  ...
  const intentGetText = async (tag) => {
    try {
      ...
      if (!raw) return null;
      // Intents are often stored as JSON objects; prefer the "text" field if present.
  ...
  const identity = (await intentGetText("identity")) || "I am Aura — an autonomous, evidence-first control-plane for ARK Systems. I help you think, plan, and operate safely without hype or coercion.";
  const capability = (await intentGetText("capability")) || "In planning mode, I help you clarify goals, organize ideas, surface tradeoffs, and outline safe next steps. In operator mode, I can run approved commands when evidence requirements are satisfied.";
  const gating = (await intentGetText("gating")) || "I avoid making status claims without verified evidence. If you ask for a gated claim, I will guide verification rather than assert outcomes.";
  ...
  if (humanMode) {
    // Planning-only response: do not echo internal intent records.
    const role = identity;
    const bullets = [
      "Clarify what you want next (one-sentence outcome).",
      "List constraints (time, money, safety, trust, dependencies).",
      "Choose the smallest safe next step that changes nothing in production.",
      "Define what evidence would be required before any real-world claim.",
      "Draft a short plan + checklist you can follow."
    ];
