...
export default {
  async fetch(request, env) {
    ...
    if ((request.method === "GET" || request.method === "HEAD") && (url.pathname === "/" || url.pathname === "/core" || url.pathname === "/ui")) {
      ...
      const mutated = await injectUiMutations(env, UI_HTML, uiHost);
      return new Response(mutated, { headers: { 'content-type': 'text/html; charset=utf-8' } });
    }
...
async function injectUiMutations(env, html, activeHost) {
  try {
    ...
    for (const it of items) {
      ...
      if (scope === "GLOBAL") { frags.push(String(it.html)); continue; }
      if (scope === "HOST") {
        const itHost = String(it.host || it.activeHost || it.domain || "").trim().toLowerCase();
        // If no explicit host stored on item, treat as current host-scoped by default.
        if (!itHost || (host && itHost === host)) frags.push(String(it.html));
        continue;
      }
      // USER scope not implemented in this probe
    }
