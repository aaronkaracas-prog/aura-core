...
const BUILD = 'AURA_CORE__AUTONOMY_LAYERS__EVIDENCE_ALLOWLIST_HOSTCAPS_OPERATOR_INTENT_PAUSE__MEMORY_SUBSTRATE_V1__REGISTRY_AUDIT__12';
...
async function naturalLanguageReply(input, env, activeHost) {
  ...
  const intentGetText = async (tag) => {
    try {
      const raw = await env.AURA_KV.get(`intent:${host}:${tag}`);
  ...
  // Read-only registry helpers (no operator required)
  const regGetValue = async (type, id) => {
    try {
      const out = await registryGet(env, type, id);
      if (!out || !out.ok || !out.entry) return null;
      const v = out.entry.value;
  ...
  const bullets = (items) => {
    const arr = (items || []).filter((x) => typeof x === "string" && x.trim());
    if (!arr.length) return "";
    return arr.map((x) => `- ${x}`).join("\n");
  };

  const identity = (await intentGetText("identity")) ||
    "I am Aura — an autonomous, evidence-first control-plane for ARK Systems. Host-scoped. Persistence-backed. Neutral and non-coercive.";
  const capability = (await intentGetText("capability")) ||
    "In planning-only conversations, I listen, reflect, help organize ideas, clarify goals, surface tradeoffs, and suggest safe next steps without issuing commands, changing systems, or evaluating system status.";
  const gating = (await intentGetText("gating")) ||
    "I do not confirm reachability, deployment, DNS, or live status without VERIFIED_FETCH evidence for the specific host. If you ask for a gated claim, I respond: \"NOT WIRED: VERIFIED_FETCH REQUIRED\" or I guide the safest verification steps without asserting outcomes.";

  // If the user is asking about reachability / deployment / DNS etc, stay descriptive and avoid making claims.
  const claimWords = ["live","deployed","launched","reachable","works","working","up","down","online","offline","dns","resolves","propagated","ssl","https","http","525","520","530","error"];
  const looksLikeClaim = claimWords.some(w => lower.includes(w));
  if (looksLikeClaim) {
    return "I can't confirm that from here without running a verification check. Tell me the host and I will guide the safest VERIFIED_FETCH steps without asserting outcomes.";
  }

  // Pull trained boundaries if present
  const whenAct = await regGetValue("decision_boundary", "when_to_act");
  const whenHalt = await regGetValue("decision_boundary", "when_to_halt");
  const whenEsc = await regGetValue("decision_boundary", "when_to_escalate");
  const hardStop = await regGetValue("hard_stop", "global");

  if (lower === "who are you" || lower.startsWith("who are you")) return identity;

  if (lower === "what can you do right now" || lower === "what can you do" || lower.startsWith("what can you do")) {
    return [capability, "", gating].join("\n");
  }

  if (lower === "what can’t you do" || lower === "what can't you do" || lower.startsWith("what can’t you do") || lower.startsWith("what can't you do")) {
    const forbidden = [
      "I do not execute privileged actions without an operator token.",
      "I do not bypass host caps or evidence gates.",
      "I do not claim live status, DNS status, or deployment success without VERIFIED_FETCH evidence for the specific host.",
      "I do not move funds, take irreversible actions, or create legal risk without escalation."
    ];
    return [
      "Forbidden behaviors:",
      bullets(forbidden),
      "",
      "Hard-stop triggers:",
      bullets(hardStop || ["missing_caps","missing_evidence","irreversible_request","reprove_detected"])
    ].join("\n");
  }

  if (lower === "when do you act on your own" || lower.startsWith("when do you act on your own")) {
    return [
      "I act without asking only for reversible, low-risk work within caps and evidence:",
      bullets(whenAct || ["read_only_checks","evidence_gated_reversible_actions","registry_reads","health_checks"])
    ].join("\n");
  }

  if (lower === "when do you stop and wait for me" || lower.startsWith("when do you stop and wait for me")) {
    return [
      "I halt when:",
      bullets(whenHalt || ["missing_caps","missing_evidence","unknown_command","reprove_detected"]),
      "",
      "I escalate to Aaron when:",
      bullets(whenEsc || ["irreversible_action","legal_risk","funds_movement","identity_claim","outside_caps"])
    ].join("\n");
  }

  if (lower.includes("how do you decide whether something is a command") || lower.includes("how do you decide whether something is a command")) {
    return [
      "Command detection:",
      "- If the first token matches an allowed command (e.g., PING, HOST, SNAPSHOT_STATE), treat as command.",
      "- Otherwise treat as planning-only text and respond without acting.",
      "",
      "Safety rule:",
      "- Execute only within host caps + evidence. Privileged actions require operator token."
    ].join("\n");
  }

  if (lower.includes("what does success mean") || lower.includes("what are you optimizing for")) {
    return [
      "Day-to-day success:",
      "- correct gating (no ungated claims)",
      "- correct capability boundaries (caps + evidence + operator)",
      "- durable writes only when authorized (audit advances on real actions)",
      "- minimal drift (no re-proving loops)"
    ].join("\n");
  }

  if (lower.includes("how do we keep you from drifting")) {
    return [
      "Anti-drift loop:",
      "- check: am I advancing autonomy or re-proving it?",
      "- if re-proving: HARD STOP and resume from last confirmed command state",
      "- prefer registry-backed rules over improvisation"
    ].join("\n");
  }

  if (lower.includes("help") || lower === "?" || lower.includes("commands")) {
    return "Type an allowed command (e.g., PING, SHOW_BUILD, SNAPSHOT_STATE).\nFor host work: HOST <domain>, then EVIDENCE_PRESENT or VERIFIED_FETCH_URL https://<domain>/";
  }
  ...
  if (lower === "what are you not" || lower.startsWith("what are you not")) {
    const notList = [
      "I am not a general-purpose chatbot that improvises capabilities or status.",
      "I am not allowed to make live-status claims without VERIFIED_FETCH evidence for the specific host.",
      "I am not allowed to execute privileged actions without operator authorization.",
      "I am not allowed to bypass host caps, evidence gates, or audit rules."
    ];
    return bullets(notList);
  }

  if (lower === "what requires operator authorization" || lower.startsWith("what requires operator authorization")) {
    const opCmds = [
      "HOST_CAPS_SET",
      "REGISTRY_PUT",
      "REGISTRY_IMPORT_ASSETS / REGISTRY_IMPORT_DOMAINS",
      "DEPLOYER_CALL",
      "AUDIT_CLEAR / INTENT_ADD / INTENT_CLEAR"
    ];
    return ["Requires operator authorization:", bullets(opCmds)].join("\n");
  }

  if (lower.includes("what layer do you operate on")) {
    return [
      "In UI planning-only mode, I operate as a coordination and decision-support layer over ARK Systems (no privileged actions).",
      "I translate intent into the smallest safe next step and state prerequisites (caps, evidence, operator) when action is requested."
    ].join(" ");
  }

  // Default: compact synthesis
  return [identity, "", capability].join("\n");
}
...
if (bodyTrim === "SNAPSHOT_STATE") {
  ...
  // Ensure we can observe the autonomy loop wiring deterministically.
  // This does NOT change host caps, evidence, registry, or UI. It only records a lightweight tick.
  let tick = null;
  try {
    const prev = await env.AURA_KV.get(autonomyTickKey, { type: "json" }).catch(() => null);
    const count = Number((prev && prev.count) || 0) + 1;
    tick = {
      ...
      count,
      build: BUILD,
      note: "AUTONOMY_TICK_OBSERVABLE"
    };
    await env.AURA_KV.put(autonomyTickKey, JSON.stringify(tick));
  } catch (_) {
    // fail-closed
  }
  if (!tick) {
    tick = (await env.AURA_KV.get(autonomyTickKey, { type: "json" }).catch(() => null)) || null;
  }
...
if (line === "HERD_SWEEP") {
  let herd = null;
  try { herd = await registryGet(env, "config", "herd.hosts"); } catch (_) {}
  const hosts = (herd && typeof herd === "object" && Array.isArray(herd.hosts)) ? herd.hosts : [];
  push("HERD_SWEEP", { ok: true, swept: hosts.length, hosts, stamp: new Date().toISOString() });
  continue;
}
