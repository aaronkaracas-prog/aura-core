...
const KNOWN_COMMANDS = [
  ...
  "SHOW_BUILD",
  "SHOW_CLAIM_GATE",
  "SHOW_ALLOWED_COMMANDS","RUN_SELF_TEST_EVIDENCE",
  "VERIFIED_FETCH_URL",
...
export default {
  async fetch(request, env) {
    ...
    // ----------------------------
        // ----------------------------
    // SELF TEST MANIFEST V1 (Autopilot Surface Freeze)
    // ----------------------------
    const SELF_TEST_MANIFEST_V1 = {
      version: "SELF_TEST_MANIFEST_V1",
      tests: [
        "ALLOWLIST_NO_DUPES",
        "SHOW_BUILD_RETURNS_OBJECT",
        "SNAPSHOT_STATE_VALID_JSON",
        "OPERATOR_GATE_ENFORCEMENT",
        "VERIFIED_FETCH_CONTRACT",
        "CORE_INJECTION_MARKERS_PRESENT",
        "UI_CHAT_PARITY_WITH_CHAT_ENDPOINT",
        "BUILD_STAMP_HEADER_MATCHES_CHAT",
        "AUTONOMY_TICK_PERSISTENCE",
        "REGISTRY_READ_PARITY",
        "AUDIT_TRAIL_PERSISTENCE",
        "MIC_PIPELINE_SMOKE",
        "IMAGE_PIPELINE_SMOKE"
      ]
    };
...
async function registryGet(env, type, id) {
  const key = registryKey(type, id);
...
const memorySchemaV1 = {
  ...
  registries: {
    ...
    ui: {
      id: "string (e.g. mic_button)",
      html: "string (html fragment)",
      scope: "GLOBAL | HOST | USER (optional)",
      updated_at: "iso string",
      audit: "KV audit events (system)"
    }
  }
...
// ============================
// TIER 10 â€” UI MUTATION ENGINE
// ============================

async function __AURA_UI_INJECT(env, html) {
  try {
    const list = await env.AURA_KV.list({ prefix: "ui_mutation:" });
    let injected = "";
    for (const key of list.keys) {
      const frag = await env.AURA_KV.get(key.name);
      if (frag) injected += "\n" + frag;
    }
