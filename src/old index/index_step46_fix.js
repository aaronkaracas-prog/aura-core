...
const BUILD_VERSION = "AURA_CORE__2026-01-22__AUTONOMY_STEP_46__UI_LOCK_STEP11__CITYGUIDE_NO_CLASS_TOKENS__01";
const BUILD_STAMP = "2026-01-22 19:40 PT";
...
function buildCityGuideBundle(versionStamp){
  const v = String(versionStamp||"CITYGUIDE_WORLD__BOOT__04");
  // Ultra-safe classic Service Worker script: avoids 'class=' tokens in inline HTML and avoids tricky quoting.
  return `
...
async function handle(request){
  ...
  if (path === "/api/searchText" && method === "POST"){
    var body = {};
    try { body = await request.json(); } catch(e) { body = {}; }
    var textQuery = body.textQuery || body.query || "";
    if(!textQuery) return json({ ok:false, error:"missing_textQuery" }, 400);

    var apiKey = (typeof PLACES_API_KEY !== "undefined") ? PLACES_API_KEY : (body._k || "");
    if(!apiKey) return json({ ok:false, error:"missing_places_api_key" }, 500);
    ...
    var r = await fetch("https://places.googleapis.com/v1/places:searchText", {
      ...
      headers:{
        ...
        "X-Goog-FieldMask":"places.id,places.displayName,places.formattedAddress,places.primaryType,places.types,places.rating,places.userRatingCount,places.photos"
      },
  ...
  if (path === "/api/placeDetails" && method === "POST"){
    var body2 = {};
    try { body2 = await request.json(); } catch(e) { body2 = {}; }
    var placeId = body2.placeId || "";
    if(!placeId) return json({ ok:false, error:"missing_placeId" }, 400);

    var apiKey2 = (typeof PLACES_API_KEY !== "undefined") ? PLACES_API_KEY : (body2._k || "");
    if(!apiKey2) return json({ ok:false, error:"missing_places_api_key" }, 500);

    var fieldMask = body2.fieldMask || "id,displayName,formattedAddress,location,photos,types,primaryType,websiteUri,regularOpeningHours";
    var r2 = await fetch("https://places.googleapis.com/v1/places/" + encodeURIComponent(placeId), {
      method:"GET",
      headers:{ "X-Goog-Api-Key": apiKey2, "X-Goog-FieldMask": fieldMask }
    });

    var data2 = {};
    try { data2 = await r2.json(); } catch(e) { data2 = {}; }
    return json(data2, r2.status);
  }

  if (path === "/api/photo" && method === "GET"){
    var name = url.searchParams.get("name") || "";
    var maxWidthPx = url.searchParams.get("maxWidthPx") || "800";
    if(!name) return new Response("missing name", { status:400 });

    var apiKey3 = (typeof PLACES_API_KEY !== "undefined") ? PLACES_API_KEY : "";
    if(!apiKey3) return new Response("missing places api key", { status:500 });

    var photoUrl = "https://places.googleapis.com/v1/" + name + "/media?maxWidthPx=" + encodeURIComponent(maxWidthPx) + "&key=" + encodeURIComponent(apiKey3);
    var r3 = await fetch(photoUrl);
    var ct = r3.headers.get("content-type") || "image/jpeg";
    return new Response(r3.body, { status:r3.status, headers:{ "content-type": ct, "cache-control":"public, max-age=86400" }});
  }

  // UI: minimal, no class attributes anywhere.
  var html =
    "<!doctype html><html><head><meta charset='utf-8'/>" +
    "<meta name='viewport' content='width=device-width, initial-scale=1'/>" +
    "<title>CityGuide.World</title>" +
    "<style>" +
    "body{margin:0;background:#0b0f14;color:#e7ecf3;font-family:system-ui;display:grid;place-items:center;height:100vh}" +
    "#card{width:min(720px,92vw);background:rgba(17,24,39,.92);border:1px solid rgba(255,255,255,.06);border-radius:16px;padding:22px;box-shadow:0 10px 40px rgba(0,0,0,.35)}" +
    "#row{display:flex;gap:10px;align-items:center;margin-top:12px}" +
    "input{flex:1;padding:12px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.08);background:#0b1220;color:#e7ecf3;outline:none}" +
    "button{padding:11px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.10);background:#1d4ed8;color:#fff;cursor:pointer}" +
    "#hint{opacity:.7;font-size:12px;margin-top:10px}" +
    "#out{margin-top:14px;display:grid;gap:10px}" +
    ".it{display:flex;gap:12px;align-items:flex-start;padding:12px;border-radius:14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06)}" +
    ".th{width:72px;height:72px;border-radius:12px;background:rgba(255,255,255,.06);flex:0 0 72px;overflow:hidden}" +
    ".th img{width:100%;height:100%;object-fit:cover;display:block}" +
    ".nm{font-weight:650}" +
    ".ad{opacity:.75;font-size:12px;margin-top:2px}" +
    "#build{position:fixed;left:14px;bottom:12px;font-size:11px;opacity:.55}" +
    "</style></head><body>" +
    "<div id='card'>" +
      "<div style='font-size:22px;font-weight:700'>CityGuide.World</div>" +
      "<div id='row'>" +
        "<input id='q' placeholder='Search a city or place' />" +
        "<button id='b'>Search</button>" +
      "</div>" +
      "<div id='hint'>Powered by Aura. Search uses Google Places.</div>" +
      "<div id='out'></div>" +
    "</div>" +
    "<div id='build'>Build: " + v + "</div>" +
    "<script>" +
    ...
    "async function go(){" +
      ...
      "if(!q.trim()) return;" +
      "var r=await fetch('/api/searchText',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({textQuery:q})});" +
      "var data=await r.json();" +
      "var places=(data.places||[]).slice(0,10);" +
      "if(!places.length){ out.textContent='No results.'; return; }" +
      "for(var i=0;i<places.length;i++){" +
        "var p=places[i];" +
        "var photo='';" +
        "try{ if(p.photos && p.photos[0] && p.photos[0].name){ photo='/api/photo?name='+encodeURIComponent(p.photos[0].name)+'&maxWidthPx=400'; } }catch(e){}" +
        "var div=document.createElement('div'); div.className='it';" +
        "var left=document.createElement('div'); left.className='th';" +
        "if(photo){ var img=document.createElement('img'); img.src=photo; left.appendChild(img); }" +
        "var right=document.createElement('div');" +
        "var nm=document.createElement('div'); nm.className='nm'; nm.textContent=(p.displayName&&p.displayName.text)?p.displayName.text:'(no name)';" +
        "var ad=document.createElement('div'); ad.className='ad'; ad.textContent=p.formattedAddress||'';" +
        "right.appendChild(nm); right.appendChild(ad);" +
...
async function handle(request){
  ...
  if (path === "/health"){
    return j({ ok:true, version: v });
  }

  // --------- API: searchText ----------
  if (path === "/api/searchText" && method === "POST"){
    const body = await request.json().catch(()=>({}));
    const textQuery = body.textQuery || body.query || "";
    if(!textQuery) return j({ ok:false, error:"missing_textQuery" }, 400);

    const apiKey = (typeof PLACES_API_KEY !== "undefined") ? PLACES_API_KEY : (body._k || "");
    if(!apiKey) return j({ ok:false, error:"missing_places_api_key" }, 500);
  ...
  if (path === "/api/placeDetails" && method === "POST"){
    ...
    if(!placeId) return j({ ok:false, error:"missing_placeId" }, 400);

    const apiKey = (typeof PLACES_API_KEY !== "undefined") ? PLACES_API_KEY : (body._k || "");
    if(!apiKey) return j({ ok:false, error:"missing_places_api_key" }, 500);

    const fieldMask = body.fieldMask || "id,displayName,formattedAddress,location,photos,types,primaryType,websiteUri,regularOpeningHours";
  ...
  if (path === "/api/photo" && method === "GET"){
    ...
    const apiKey = (typeof PLACES_API_KEY !== "undefined") ? PLACES_API_KEY : "";
    if(!apiKey) return new Response("missing places api key", { status:500 });

    const photoUrl = "https://places.googleapis.com/v1/" + name + "/media?maxWidthPx=" + encodeURIComponent(maxWidthPx) + "&key=" + encodeURIComponent(apiKey);
    const r = await fetch(photoUrl);
    const ct = r.headers.get("content-type") || "image/jpeg";
    return new Response(r.body, { status:r.status, headers:{ "content-type": ct, "cache-control":"public, max-age=86400" }});
  }
  ...
  const page =
    ...
    ".item{display:flex;gap:12px;align-items:flex-start;padding:12px;border-radius:14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06)}" +
    ".thumb{width:72px;height:72px;border-radius:12px;background:rgba(255,255,255,.06);flex:0 0 72px;overflow:hidden}" +
    ".thumb img{width:100%;height:100%;object-fit:cover;display:block}" +
    ".name{font-weight:650}" +
    ".addr{opacity:.75;font-size:12px;margin-top:2px}" +
    ".build{position:fixed;left:14px;bottom:12px;font-size:11px;opacity:.55}" +
    "</style></head><body>" +
    "<div class='card'>" +
      "<div style='font-size:22px;font-weight:700'>CityGuide.World</div>" +
      ...
      "</div>" +
      "<div class='hint'>Powered by Aura. Search uses Google Places.</div>" +
      "<div id='out' class='results'></div>" +
    "</div>" +
    "<div class='build'>Build: " + v + "</div>" +
    "<script>" +
    "async function go(){" +
      "const q=document.getElementById('q').value||'';" +
      "document.getElementById('out').innerHTML='';" +
      "if(!q.trim()) return;" +
      "const r=await fetch('/api/searchText',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({textQuery:q})});" +
      "const data=await r.json();" +
      "const places=(data.places||[]).slice(0,10);" +
      "const out=document.getElementById('out');" +
      "if(!places.length){out.innerHTML='<div class=hint>No results.</div>';return;}" +
      "for(const p of places){" +
        ...
        "div.innerHTML=" +
          ...
          "\"<div><div class='name'>\"+(p.displayName&&p.displayName.text?p.displayName.text:'(no name)')+\"</div>\"+" +
          "\"<div class='addr'>\"+(p.formattedAddress||'')+\"</div></div>\";" +
        "out.appendChild(div);" +
      "}" +
    "}" +
    "</script>" +
...
function asciiSafe(str){
  // Ensure the generated CityGuide bundle contains only ASCII characters.
  // Cloudflare validation can fail on certain unicode tokens (e.g., U+2028/U+2029) or smart punctuation.
  let out = "";
  for (let i = 0; i < str.length; i++){
    const c = str.charCodeAt(i);
    if (c === 0x2028) { out += "\u2028"; continue; }
    if (c === 0x2029) { out += "\u2029"; continue; }
    if (c <= 0x7f) { out += str[i]; continue; }
...
function safeB64(text){
  ...
  for (let i=0; i<enc.length; i+=chunk){
    bin += String.fromCharCode(...enc.slice(i, i+chunk));
  }
  return btoa(bin);
}
...
async function handleCommand(cmd, env, ctx){
  ...
  try {
    ...
    const bundleTextRaw = buildCityGuideBundle("CITYGUIDE_WORLD__" + BUILD_VERSION);
    const bundleText = asciiSafe(bundleTextRaw);
    const bundle_b64 = safeB64(bundleText);
    const prep = { target:"prod", script_name:"cityguide-world", bundle_b64, promotion_phrase, build: BUILD_VERSION, raw_bytes: bundleTextRaw.length, bundle_bytes: bundleText.length };
    await kvPutJson(env, KV_KEYS.CITYGUIDE_PREP, prep);
    await cityguideSetStatus(env, { phase:"prepared", ok:true, raw_bytes: prep.raw_bytes, bundle_bytes: prep.bundle_bytes, b64_bytes: bundle_b64.length });
    return {
      ok:true,
      reply:"Prepared CityGuide bundle. Now run COMMIT_CITYGUIDE.",
      prepared:{
        script_name:"cityguide-world",
        target:"prod",
        promotion_phrase,
        ...
        b64_bytes: bundle_b64.length
      }
    };
  } catch (e) {
    await cityguideSetStatus(env, { phase:"failed", ok:false, error:"prepare_failed", detail:String(e) });
    return { ok:false, reply:"Prepare failed.", error:"prepare_failed", detail:String(e) };
  }
}
