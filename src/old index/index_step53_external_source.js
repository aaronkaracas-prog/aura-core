...
const BUILD_VERSION = "AURA_CORE__2026-01-22__AUTONOMY_STEP_53__CITYGUIDE_EXTERNAL_SOURCE_MODULE_LOCK__01";
...
const KV_KEYS = {
  ...
  CITYGUIDE_SOURCE: "aura:cityguide:source"
};
...
/* =========================
   CityGuide.Worfunction truncate(s, n){ s = String(s||""); return s.length > n ? (s.slice(0,n) + "…") : s; }

/* =========================
   CityGuide.World bundle (EXTERNAL SOURCE FIRST)
   =========================
   IMPORTANT:
   - Cloudflare deploy path we use ACCEPTS MODULE format.
   - Service-worker handler format triggered error 10068.
   - Therefore, CityGuide bundles MUST be module syntax:
       export default { fetch(request, env, ctx) { ... } }
*/

async function cityguideGetSource(env){
  if(!kvOk(env)) return { ok:false, error:"no_kv" };
  const url = await kvGetText(env, KV_KEYS.CITYGUIDE_SOURCE);
  return { ok:true, url: (url||"").trim() };
}

async function cityguideSetSource(env, url){
  if(!kvOk(env)) return { ok:false, error:"no_kv" };
  const u = String(url||"").trim();
  if(!u) return { ok:false, error:"missing_url" };
  // Basic sanity (do NOT hard-fail on non-https, but strongly prefer https)
  await kvPutText(env, KV_KEYS.CITYGUIDE_SOURCE, u);
  return { ok:true, url: u };
}

function buildCityGuideDefaultModule(versionStamp){
  const v = String(versionStamp||"CITYGUIDE_DEFAULT__01");
  return `export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);
    if (url.pathname === "/health") {
      return new Response(JSON.stringify({ ok:true, version:"${v}", format:"module", note:"default_fallback" }), {
        headers: { "content-type":"application/json; charset=utf-8" }
      });
    }
    const html = "<!doctype html><html><head><meta charset=\\"utf-8\\"><meta name=\\"viewport\\" content=\\"width=device-width,initial-scale=1\\"><title>CityGuide.World</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#0b0f14;color:#e7ecf3;font-family:system-ui}a{color:inherit}</style></head><body><div><h1>CityGuide.World</h1><p>Deployed default fallback bundle.</p><p>Next: set external source URL via Aura.</p></div></body></html>";
    return new Response(html, { headers: { "content-type":"text/html; charset=utf-8" } });
  }
};`;
}

async function fetchCityGuideBundleFromUrl(url){
  const u = String(url||"").trim();
  if(!u) return { ok:false, error:"missing_url" };
  let r, text;
  try {
  ...
  if(!r.ok) return { ok:false, error:"fetch_http_error", status:r.status, detail:truncate(text, 300) };

  const body = String(text||"");
  // Hard lock: module format required
  if(!/export\s+default\s*\{/.test(body) && !/export\s+default\s+class/.test(body)) {
    return { ok:false, error:"not_module_format", hint:"CityGuide bundle must start with `export default { ... }`", head: truncate(body, 160) };
  }
  return { ok:true, bundle: body, bytes: body.length };
}
...
async function handleCommand(cmd, env, ctx){
  ...
  if (c.startsWith("CITYGUIDE_SET_SOURCE:") || c.startsWith("CITYGUIDE_SET_SOURCE ")) {
    ...
    const res = await cityguideSetSource(env, url);
    return res.ok ? { ok:true, reply:"CityGuide source URL saved.", url: res.url } : { ok:false, error: res.error, detail: res.detail||null, hint: res.hint||null };
  }

  if (c === "CITYGUIDE_SHOW_SOURCE") {
    const res = await cityguideGetSource(env);
    ...
    if (ctx && ctx.waitUntil) {
      ctx.waitUntil((async () => {
        try {
          ...
          await cityguideSetStatus(env, { phase:"deployed", ok:true, status:r.status, body, error:null });
        } catch (e) {
...
if (c === "PREPARE_CITYGUIDE") {
    const promotion_phrase = env.PROMOTE_PHRASE || "PROMOTE_TO_PROD";
    const source = await cityguideGetSource(env);
    let bundleText = null;
    let bundleMeta = { mode:"default_fallback" };

    if (source && source.ok && source.url) {
      const fetched = await fetchCityGuideBundleFromUrl(source.url);
      if (!fetched.ok) {
        await cityguideSetStatus(env, { phase:"failed", ok:false, error:"source_fetch_failed", detail:fetched, note:"CityGuide source URL fetch failed. Fix URL or bundle format." });
        return { ok:false, reply:"Prepare failed (source fetch).", error:"source_fetch_failed", detail:fetched };
      }
      bundleText = fetched.bundle;
      bundleMeta = { mode:"external_url", url: source.url, bytes: fetched.bytes };
    } else {
      bundleText = buildCityGuideDefaultModule("CITYGUIDE_DEFAULT__"+BUILD_VERSION);
      bundleMeta = { mode:"default_fallback" };
    }
    ...
    const prep = {
      ...
      promotion_phrase,
      build: BUILD_VERSION,
      bundle_bytes: bundleText.length,
      source: bundleMeta
    };

    await kvPutJson(env, KV_KEYS.CITYGUIDE_PREP, prep);
    await cityguideSetStatus(env, {
      phase:"prepared",
      ok:true,
      bundle_bytes: bundleText.length,
      b64_bytes: bundle_b64.length,
      note:"Bundle prepared in KV (non-blocking). Run COMMIT_CITYGUIDE to deploy.",
      source: bundleMeta
    });

    return {
      ok:true,
      reply: bundleMeta.mode === "external_url"
        ? "Prepared CityGuide bundle from external source. Now run COMMIT_CITYGUIDE."
        : "Prepared CityGuide default fallback bundle. Now run COMMIT_CITYGUIDE.",
      prepared:{ script_name:"cityguide-world", target:"prod", promotion_phrase, source: bundleMeta }
    };
  }
...
if (c === "DRYRUN_LAUNCH_CITYGUIDE") {
    const prep = await kvGetJson(env, KV_KEYS.CITYGUIDE_PREP);
    const source = await cityguideGetSource(env);
    if (prep && prep.bundle_b64) {
      return {
        ok:true,
        using:"prepared_bundle",
        source_url: (source && source.ok) ? source.url : "",
        would_send:{
          target: prep.target,
          script_name: prep.script_name,
          promotion_phrase: prep.promotion_phrase,
          bundle_b64_bytes: prep.bundle_b64.length,
          bundle_bytes: prep.bundle_bytes || null
        }
      };
    }
    // No prepared bundle yet; show what would happen
    return {
      ok:true,
      using:"no_prepared_bundle",
      source_url: (source && source.ok) ? source.url : "",
      hint:"Run PREPARE_CITYGUIDE first."
    };
  }
