...
const BUILD_VERSION = "AURA_CORE__2026-01-22__AUTONOMY_STEP_53__UI_LOCK_STEP11__CITYGUIDE_BUNDLE_ARRAY_SAFE__01";
const BUILD_STAMP = "2026-01-22 22:05 PT";
...
function buildCityGuideBundle() {
  // Hardened bundle array: avoids truncation and keeps JS parser happy.
  const lines = [
  ...
  "  if(path===\"/health\"){ return j({ok:true, version: \"CITYGUIDE_BUNDLE_SAFE__01\"}); }",
  ...
  "    var apiKey = body.apiKey || body._k || \"\";",
  ...
  "    \"<style>body{margin:0;background:#0b0f14;color:#e7ecf3;font-family:system-ui;display:grid;place-items:center;height:100vh}\" +",
  ...
  "      \"</div>\" +",
  "      \"<div id=\\"hint\\">Search uses Google Places.</div>\" +",
  "      \"<div id=\\"out\\"></div>\" +",
  "    \"</div>\" +",
  "    \"<script>\" +",
  "      \"function esc(s){return (s||\\"\\").replace(/&/g,\\"&amp;\\").replace(/</g,\\"&lt;\\").replace(/>/g,\\"&gt;\\");}\" +",
  "      \"async function go(){var q=document.getElementById(\\"q\\").value||\\"\\";var out=document.getElementById(\\"out\\");out.innerHTML=\\"\\";if(!q.trim())return;\" +",
  "      \"var r=await fetch(\\"/api/searchText\\",{method:\\"POST\\",headers:{\\"content-type\\":\\"application/json\\"},body:JSON.stringify({textQuery:q,apiKey:\\"\\"})});\" +",
  "      \"var data=await r.json();var places=(data.places||[]).slice(0,10);if(!places.length){out.textContent=\\"No results.\\";return;}\" +",
  "      \"for(var i=0;i<places.length;i++){var p=places[i];var name=(p.displayName&&p.displayName.text)?p.displayName.text:\\"(no name)\\";var addr=p.formattedAddress||\\"\\";\" +",
  "      \"var div=document.createElement(\\"div\\");div.style.padding=\\"12px\\";div.style.border=\\"1px solid rgba(255,255,255,.06)\\";div.style.borderRadius=\\"14px\\";div.style.background=\\"rgba(255,255,255,.04)\\";\" +",
  "      \"div.innerHTML=\\"<div style=\\\\"font-weight:650\\\\">\\"+esc(name)+\\"</div><div style=\\\\"opacity:.75;font-size:12px;margin-top:2px\\\\">\\"+esc(addr)+\\"</div>\\";out.appendChild(div);}\" +",
  "      \"}\" +",
