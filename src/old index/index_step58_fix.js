...
const BUILD_VERSION = "AURA_CORE__2026-01-22__AUTONOMY_STEP_58__UI_LOCK_STEP11__CITYGUIDE_MODULE_JOIN_FIX__01";
const BUILD_STAMP = "2026-01-22 22:35 PT";
...
function buildCityGuideBundle() {
  // Module worker bundle (avoids Cloudflare "no registered event handlers" 10068).
  const lines = ["// CITYGUIDE.WORLD module worker (validator-safe)", "const VERSION = \"CITYGUIDE_WORLD__AURA_CORE__STEP57__01\";", "function json(obj,status){", "  return new Response(JSON.stringify(obj),{status:status||200,headers:{\"content-type\":\"application/json; charset=utf-8\"}});", "}", "function html(body,status){", "  return new Response(body,{status:status||200,headers:{\"content-type\":\"text/html; charset=utf-8\"}});", "}", "async function handle(request, env){", "  const url = new URL(request.url);", "  const path = url.pathname;", "  if(path===\"/health\"){ return json({ok:true, version: VERSION}); }", "  if(path===\"/api/searchText\" && request.method===\"POST\"){", "    let body={}; try{ body=await request.json(); }catch(e){ body={}; }", "    const textQuery = body.textQuery || body.query || \"\";", "    if(!textQuery) return json({ok:false,error:\"missing_textQuery\"},400);", "    const apiKey = env && env.PLACES_API_KEY ? env.PLACES_API_KEY : (body.apiKey || body._k || \"\");", "    if(!apiKey) return json({ok:false,error:\"missing_places_api_key\"},500);", "    const r = await fetch(\"https://places.googleapis.com/v1/places:searchText\",{", "      method:\"POST\",", "      headers:{", "        \"content-type\":\"application/json\",", "        \"X-Goog-Api-Key\": apiKey,", "        \"X-Goog-FieldMask\":\"places.id,places.displayName,places.formattedAddress,places.primaryType,places.types,places.rating,places.userRatingCount,places.photos\"", "      },", "      body: JSON.stringify({textQuery})", "    });", "    let data={}; try{ data=await r.json(); }catch(e){ data={}; }", "    return json(data, r.status);", "  }", "  // Minimal black UI (no class tokens required)", "  const page = \"\" +", "    \"<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'>\" +", "    \"<title>CityGuide.World</title>\" +", "    \"<style>body{margin:0;background:#0b0f14;color:#e7ecf3;font-family:system-ui;display:grid;place-items:center;height:100vh}\" +", "    \"#c{width:min(760px,92vw);background:rgba(17,24,39,.92);border:1px solid rgba(255,255,255,.06);border-radius:16px;padding:22px}\" +", "    \"#r{display:flex;gap:10px;align-items:center;margin-top:12px}\" +", "    \"input{flex:1;padding:12px;border-radius:12px;border:1px solid rgba(255,255,255,.08);background:#0b1220;color:#e7ecf3;outline:none}\" +", "    \"button{padding:11px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.10);background:#1d4ed8;color:#fff;cursor:pointer}\" +", "    \"#o{margin-top:14px;display:grid;gap:10px}\" +", "    \"</style></head><body>\" +", "    \"<div id='c'><div style='font-size:22px;font-weight:700'>CityGuide.World</div>\" +", "    \"<div id='r'><input id='q' placeholder='Search a city or place'><button id='b'>Search</button></div>\" +", "    \"<div id='o'></div></div>\" +", "    \"<script>\" +", "    \"function esc(s){return (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}\" +", "    \"async function go(){var q=document.getElementById('q').value||'';var o=document.getElementById('o');o.innerHTML='';if(!q.trim())return;\" +", "    \"var r=await fetch('/api/searchText',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({textQuery:q})});\" +", "    \"var d=await r.json();var p=(d.places||[]).slice(0,10);if(!p.length){o.textContent='No results.';return;}\" +", "    \"for(var i=0;i<p.length;i++){var it=p[i];var nm=(it.displayName&&it.displayName.text)?it.displayName.text:'(no name)';var ad=it.formattedAddress||'';\" +", "    \"var div=document.createElement('div');div.style.padding='12px';div.style.border='1px solid rgba(255,255,255,.06)';div.style.borderRadius='14px';div.style.background='rgba(255,255,255,.04)';\" +", "    \"div.innerHTML='<div style=\"font-weight:650\">' + esc(nm) + '</div><div style=\"opacity:.75;font-size:12px;margin-top:2px\">' + esc(ad) + '</div>';\" +", "    \"o.appendChild(div);}\" +", "    \"}\" +", "    \"document.getElementById('b').addEventListener('click',go);\" +", "    \"document.getElementById('q').addEventListener('keydown',function(e){if(e.key==='Enter')go();});\" +", "    \"</script></body></html>\";", "  return html(page);", "}", "export default {", "  fetch(request, env){", "    return handle(request, env);", "  }", "};"];
  return lines.join("
");
}
